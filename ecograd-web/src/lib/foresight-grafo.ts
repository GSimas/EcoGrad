/**
 * A parte do Radar de Foresight que monta grafo de coocorrência: bootstrap do
 * betweenness, backtest histórico e Grid Search. Transcrição fiel de backend.py:
 *   calcular_betweenness_bootstrap (169) · validar_foresight_historico (328)
 *   otimizar_parametros_foresight (248)
 *
 * Só o worker de SNA (e o harness de paridade) importa este módulo; a página
 * usa `foresight-math`, que não depende do `graphology`.
 */
import Graph from 'graphology';
import type { BacktestRow, BootstrapMap, Documento, GridSearchRow, TipoForesight } from '@/types';
import { betweennessCentrality, compactar } from './graph-core';
import { median, quantile, SeededRandom, std } from './stats';
import {
  classificarPorPercentil,
  contar,
