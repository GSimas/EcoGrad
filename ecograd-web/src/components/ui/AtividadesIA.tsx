import { rotaVisivel } from '@/lib/navigation';
...
export function AtividadesIA(){
 ...
 return <section aria-label="Atividades de IA" className="m-4 space-y-3 rounded-lg border border-eco-border bg-eco-panel p-3 text-sm">
  ...
  {ativo&&<div className="flex flex-wrap gap-2">{rotaVisivel('memetica')&&<button className="btn" onClick={()=>navigatePage('memetica')}>Abrir lote de ontologia ({lote.itens.filter(i=>i.estado==='concluido').length}/{lote.itens.length})</button>}<button className="btn" onClick={interromperExtracao}>Interromper lote de ontologia</button></div>}
