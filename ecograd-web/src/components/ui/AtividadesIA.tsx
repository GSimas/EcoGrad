...
export function AtividadesIA(){
 ...
 return <section aria-label="Atividades de IA" className="m-4 space-y-3 rounded-lg border border-eco-border bg-eco-panel p-3 text-sm">
  ...
  {chat&&<div className="flex flex-wrap gap-2"><button className="btn" onClick={()=>useEcoGradStore.setState(s=>({ui:{...s.ui,'consultor.aberto':true}}))}>Abrir conversa em andamento</button><button className="btn" onClick={interromperConversa}>Interromper resposta do consultor</button></div>}
