  ...
  return (
    <section className="space-y-4">
      ...
      <Expander titulo="Síntese da amostra por IA" icone={<Sparkles size={16} aria-hidden />} persistir={false}>
        ...
        <p className="text-sm text-slate-300">Etapa opcional. Envia à DeepSeek os nomes das coleções e títulos/palavras-chave de até 25 documentos, espaçados na ordem da seleção (salto {amostra.salto}). Amostra atual: {amostra.quantidade} de {amostra.total} registros, {amostra.texto.length} caracteres. {amostra.truncada?'O texto foi cortado em 20.000 caracteres, podendo terminar no meio de um registro.':'Não houve corte por tamanho.'} Não envia resumos, chat ou dados CAPES. A amostra não é aleatória nem garante representação de todas as coleções.</p>
