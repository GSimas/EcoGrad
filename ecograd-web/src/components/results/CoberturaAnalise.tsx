...
/**
 * `emDica`: o texto vai para uma dica "i" em vez de ocupar a página. O rótulo do
 * botão é o mesmo da seção, e é por ele que o tour guiado encontra a cobertura.
 */
export function CoberturaAnalise({ docs, emDica = false, introducao }: { docs: readonly Documento[]; emDica?: boolean; introducao?: ReactNode }) {
  ...
  if (emDica) return <Dica rotulo="Cobertura do recorte">{conteudo}</Dica>;
  ...
  return <section aria-label="Cobertura do recorte" className="space-y-2 text-xs leading-relaxed text-slate-300">{conteudo}</section>;
}
