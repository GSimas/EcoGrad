...
/**
 * `mostrarResumo`: o Dashboard lista só a ficha; o resumo fica a um clique, em "Ler resumo e detalhes".
 * `semCabecalho`: dentro de uma janela, o título dela já nomeia a lista.
 */
export function Trabalhos({ docs, sessionKey, titulo = 'Trabalhos para ler', mostrarResumo = true, semCabecalho = false }: { docs: readonly Documento[]; sessionKey: string; titulo?: string; mostrarResumo?: boolean; semCabecalho?: boolean }) {
  const id = useId();
  ...
  return <section className="space-y-4" aria-label={titulo}>
    {!semCabecalho && <h2 className="text-xl font-semibold">{titulo}</h2>}
    ...
    {!result.length ? <p className="rounded-lg border border-eco-border p-4 text-sm">Nenhum trabalho neste filtro. Limpe os filtros para voltar ao conjunto disponível.</p> : <ol className="grid gap-3 md:grid-cols-2 lg:grid-cols-3" start={paginaAtual * 6 + 1}>{result.slice(paginaAtual * 6, paginaAtual * 6 + 6).map((d, i) => <li key={paginaAtual * 6 + i} className="card flex h-full flex-col gap-2">
      ...
      {mostrarResumo && <TextoDoAcervo texto={d.resumo} className="line-clamp-4 text-xs leading-relaxed text-slate-400" vazio="Resumo não disponível no recorte local." />}
