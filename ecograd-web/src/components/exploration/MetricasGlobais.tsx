...
import { Aviso, Card, Expander, Kpi } from '@/components/ui/primitives';
...
export function MetricasGlobais() {
  ...
  return (
    <section className="space-y-4">
      <div>
        <h2 className="flex items-center gap-2 text-lg font-semibold"><Activity size={18} aria-hidden /> Métricas de redes complexas do grafo global</h2>
        <p className="mt-1 text-sm text-slate-400">
          Densidade, eficiência, entropia e influência estrutural da rede que liga documentos a autores, orientadores e palavras-chave.
        </p>
      </div>
      ...
      <Card className="space-y-3">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <p className="text-xs text-slate-500">
            Eficiência global, PageRank, eigenvector e restrição de Burt rodam em segundo plano. Você pode navegar durante o cálculo.
          </p>
        </div>
      ...
      <Expander titulo="O que estes índices descrevem">
      ...
      {metricas && (
        <>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <Card className="space-y-2">
              <p className="text-sm font-medium text-slate-200">Influência e redundância estrutural</p>
              <div className="grid gap-2 sm:grid-cols-2">
                ...
                <Kpi rotulo="Redundância" valor={metricas.redundancia.toFixed(4)} detalhe="1 − eficiência global" className="sm:col-span-2" />
              </div>
            </Card>
          </div>
        </>
      ...
      <div className="space-y-3">
        <h3 className="text-sm font-semibold text-slate-200">Ecologia profunda do grafo global</h3>
        <p className="text-xs text-slate-500">
          Calculada junto com a rede global, assim que as coleções são carregadas — não precisa de um botão.
        </p>
        {!maturidade ? (
          <Aviso tipo="aviso">
