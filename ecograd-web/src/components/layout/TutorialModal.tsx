...
const PASSOS: Passo[] = [
  ...
  { icone: BotMessageSquare, titulo: 'Conversar e conferir as fontes', corpo: <><p>Abra o <strong>Consultor IA</strong> pelo botão no canto inferior direito, usando a chave de API do provedor que você escolher (OpenAI, Anthropic, Google, OpenRouter e outros). Ele recebe contexto limitado da análise, que pode incluir uma amostra do catálogo. Descreva seu objetivo e confira títulos, pessoas e links nas fontes originais.</p><p>A resposta não substitui a leitura dos trabalhos. Uma informação ausente no contexto enviado à IA não significa que ela inexista no acervo. A conversa e o texto ainda não enviado permanecem na sessão.</p></> },
