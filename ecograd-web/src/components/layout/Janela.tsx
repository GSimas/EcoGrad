...
export function Janela({ titulo, descricao, children, trigger, aberta, onOpenChange, larga = false }: {
  ...
  return <Dialog.Root open={aberta} onOpenChange={onOpenChange}>
    ...
    <Dialog.Portal>
      ...
      <Dialog.Content aria-modal="true" className={`eco-dialog-content fixed left-1/2 top-1/2 z-50 flex max-h-[92dvh] w-[calc(100%-1rem)] -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-xl border border-eco-border bg-eco-bg shadow-xl ${larga ? 'max-w-6xl' : 'max-w-lg'}`}>
