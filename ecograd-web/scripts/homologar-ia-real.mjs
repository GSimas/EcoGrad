...
if (!process.argv.includes('--executar')) throw new Error('Use --executar: até 2 pedidos locais (4 com --bloco14), com retries/fallback do servidor e possível consumo de quota.');
...
// O Consultor IA usa a chave do próprio usuário (BYOK) direto do navegador; não há função de chat a homologar aqui.
const first=await run('gemini-synthesize',{nomesProgramas:[d.programa_origem],recorte:{quantidade:1,total:1,salto:1,truncada:false},amostraTextos:`Título: ${d.titulo}\nPalavras-chave: ${(d.palavras_chave??[]).join(', ')}`});
if(first.status===200) {
