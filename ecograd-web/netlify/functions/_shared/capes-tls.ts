import { Agent, get } from 'node:https';
import { rootCertificates } from 'node:tls';

/**
 * A CAPES omite este intermediário na cadeia TLS. Certificado público oficial:
 * https://secure.globalsign.com/cacert/rnpicpedugr46ovtlsca2025.crt
 * RNP ICPEdu GR46 OV TLS CA 2025, válido até 19/11/2030.
 * Assinatura verificada contra GlobalSign Root R46, já confiável no Node.
 * Aplicado apenas às requisições CAPES; validação de certificado e hostname ativa.
 */
const INTERMEDIARIO_CAPES = `-----BEGIN CERTIFICATE-----

Lyso4S/KfU1hV0inHITEfil4
-----END CERTIFICATE-----
`;
const agenteCapes = new Agent({ ca: [...rootCertificates, INTERMEDIARIO_CAPES] });

export function carregarPaginaCapes(url: URL): Promise<unknown> {
  return new Promise((resolve, reject) => {
    const req = get(url, {
      agent: agenteCapes,
      headers: { Accept: 'application/json' },
      signal: AbortSignal.timeout(10000),
    }, (res) => {
      if (res.statusCode !== 200) {
        res.resume();
        reject(new Error(`A CAPES respondeu com HTTP ${res.statusCode}.`));
        return;
      }
      res.setEncoding('utf8');
      let body = '';
      res.on('data', (chunk: string) => { body += chunk; });
      res.on('error', reject);
      res.on('end', () => {
        try { resolve(JSON.parse(body)); }
        catch { reject(new Error('A CAPES retornou uma resposta inválida.')); }
      });
    });
    req.on('error', reject);
  });
}
