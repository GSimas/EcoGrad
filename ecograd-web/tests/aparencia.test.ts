...
test('invalid or old appearance preferences fall back without admitting arbitrary fields', () => {
  assert.deepEqual(validarAparencia({tema:'invalido',densidade:'compacta',movimento:'reduzido',rascunho:'privado'}), {tema:'escuro',densidade:'compacta',movimento:'reduzido'});
  assert.deepEqual(validarAparencia(null), {tema:'escuro',densidade:'confortavel',movimento:'sistema'});
});
test('changing appearance never changes documents, drafts, IA, history UI or worker states, including storage failure', () => {
  ...
  useAparencia.getState().definir({tema:'claro',densidade:'compacta',movimento:'reduzido'});
...
test('both theme token palettes meet text 4.5:1 and control/focus 3:1 against their surfaces', () => {
  ...
  for(const selector of [':root {', ':root[data-tema="claro"] {']) {
